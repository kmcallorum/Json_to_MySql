import { FieldAnalysis, SchemaAnalysis } from '../types';
...
export class JsonAnalyzer {
   ...
   */
  analyze(documents: any[], sampleSize?: number): SchemaAnalysis {
    ...
    const fields: FieldAnalysis[] = Array.from(fieldStats.entries()).map(([path, stats]) => {
      return {
        path,
        types: stats.types,
   ...
   */
  private suggestTableName(path: string): string {
   ...
   */
  private suggestDataType(stats: any): string {
    const types = Array.from(stats.types) as string[];
    ...
    if (type === 'number') {
      ...
      const hasDecimals = stats.samples.some((s: number) => s % 1 !== 0);
