...
async function setupApiMocks(page: any) {
  await page.route('**/api/analysis/test-connection', async (route: any) => {
    await route.fulfill({
      status: 200,
      contentType: 'application/json',
  ...
  await page.route('**/api/analysis/discover-fields', async (route: any) => {
    await route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify({
  ...
  await page.route('**/api/analysis/analyze', async (route: any) => {
    await route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify({
  ...
  await page.route('**/api/filters/list', async (route: any) => {
    await route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify({
  ...
  await page.route('**/api/mappings/save', async (route: any) => {
    await route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify({
  ...
  await page.route('**/api/mappings/execute', async (route: any) => {
    await route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify({
...
async function testDatabaseConnection(page: any) {
...
async function discoverFields(page: any) {
...
async function buildWhereConditions(page: any) {
...
async function analyzeJsonStructure(page: any) {
...
async function mapFieldsToTables(page: any) {
...
async function defineRelationships(page: any) {
...
async function executeFlatteningOperation(page: any) {
  const executeBtn = page.getByRole('button', { name: /Execute|Flatten|Process/i });
  if (await executeBtn.isVisible()) {
    await executeBtn.click();
  }
...
async function saveConfiguration(page: any, configName: string) {
...
async function loadConfiguration(page: any, configName: string) {
