...
describe('SqlGenerator', () => {
    ...
    it('should render component with title', () => {
        render(_jsx(SqlGenerator, { tables: mockTables, mappings: mockMappings, baseTableName: "test_table" }));
    ...
    it('should display relationships in SQL comments', () => {
        render(_jsx(SqlGenerator, { tables: mockTables, mappings: mockMappings, baseTableName: "test_table", relationships: mockRelationships }));
    ...
    it('should execute flattening when confirmed', async () => {
        global.confirm.mockReturnValue(true);
        api.executeFlattening.mockResolvedValue({
            success: true,
    ...
    it('should show executing state when flattening', async () => {
        global.confirm.mockReturnValue(true);
        api.executeFlattening.mockImplementation(() => new Promise(resolve => setTimeout(resolve, 1000)));
        render(_jsx(SqlGenerator, { tables: mockTables, mappings: mockMappings, baseTableName: "test_table" }));
        const executeButton = screen.getByText(/Execute Now/);
        fireEvent.click(executeButton);
        await waitFor(() => {
    ...
    it('should handle execution exception', async () => {
        global.confirm.mockReturnValue(true);
        api.executeFlattening.mockRejectedValue(new Error('Network error'));
        render(_jsx(SqlGenerator, { tables: mockTables, mappings: mockMappings, baseTableName: "test_table" }));
        const executeButton = screen.getByText(/Execute Now/);
        fireEvent.click(executeButton);
        await waitFor(() => {
    ...
    it('should show relationship confirmation message', async () => {
        ...
        const executeButton = screen.getByText(/Execute Now/);
        fireEvent.click(executeButton);
        expect(global.confirm).toHaveBeenCalledWith(expect.stringContaining('Execute flattening with 1 relationship(s)?'));
        expect(global.confirm).toHaveBeenCalledWith(expect.stringContaining('users → posts'));
    });
    it('should include base table name in generated SQL header', () => {
        render(_jsx(SqlGenerator, { tables: mockTables, mappings: mockMappings, baseTableName: "my_custom_table" }));
