...
describe('TableSelector', () => {
  ...
  it('should render with title', () => {
    render(
      <TableSelector
        suggestedTables={['users', 'orders']}
        fields={mockFields}
        baseTableName="test_table"
        onTablesSelected={mockOnTablesSelected}
      />
    );
  ...
  it('should switch to existing tables mode', async () => {
    (api.getTableList as any).mockResolvedValue({
      success: true,
      tables: []
    });
  ...
  it('should show loading state when fetching existing tables', async () => {
    (api.getTableList as any).mockImplementation(() => new Promise(resolve => setTimeout(resolve, 1000)));
  ...
  it('should disable load button when no tables selected', async () => {
    ...
    await waitFor(() => {
      const loadButton = screen.getByText(/Load Selected Tables \(0\)/) as HTMLButtonElement;
  ...
  it('should check for existing mappings when loading tables', async () => {
    ...
    (api.findMappingsByTables as any).mockResolvedValue({
      success: true,
  ...
  it('should load mapping when Load This Mapping clicked', async () => {
    ...
    (api.loadMappingConfig as any).mockResolvedValue({
      success: true,
      config: {
  ...
  it('should skip mapping and load tables manually', async () => {
    ...
    (api.getTableStructures as any).mockResolvedValue({
      success: true,
  ...
  it('should create custom table when prompted', () => {
    (global.prompt as any).mockReturnValue('my_custom_table');
  ...
  it('should handle error when loading tables fails', async () => {
    (api.getTableList as any).mockRejectedValue(new Error('Network error'));
  ...
  it('should handle duplicate table names in suggested tables', () => {
    render(
      <TableSelector
        suggestedTables={['users', 'users', 'orders', 'orders']}
        fields={mockFields}
        baseTableName="test_table"
        onTablesSelected={mockOnTablesSelected}
      />
